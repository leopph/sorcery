...
namespace leopph
{
	...
	class Transform final : public Component
	{
		public:
			...
			LEOPPHAPI
			auto Translate(const Vector3& vector, Space base = Space::World) -> void;
			...
			LEOPPHAPI
			auto Rotate(const Quaternion& rotation, Space base = Space::World) -> void;
			...
			LEOPPHAPI
			auto Rotate(const Vector3& axis, float amountDegrees, Space base = Space::World) -> void;
			...
			LEOPPHAPI
			auto Rescale(const Vector3& scaling, Space base = Space::World) -> void;
			...
			[[nodiscard]] constexpr
			auto Forward() const noexcept -> const Vector3&;

			// Get the Transform's current right, X vector in World Space.
			[[nodiscard]] constexpr
			auto Right() const noexcept -> const Vector3&;

			// Get the Transform's current up, Y vector in World Space.
			[[nodiscard]] constexpr
			auto Up() const noexcept -> const Vector3&;
			...
			LEOPPHAPI
			auto Parent(const leopph::Entity* parent) -> void;
			...
			LEOPPHAPI
			auto Matrices() const -> const std::pair<Matrix4, Matrix4>&;
			...
			LEOPPHAPI explicit Transform(const Vector3& pos = Vector3{},
