...
namespace leopph::internal
{
	...
	auto GeometryBuffer::InitBuffers() noexcept -> void
	{
		...
		glTextureStorage2D(m_Textures[NORM_TEX], 1, GL_RG16F, m_Res[0], m_Res[1]);
		...
		glNamedFramebufferTexture(m_FrameBuffer, GL_COLOR_ATTACHMENT0, m_Textures[NORM_TEX], 0);
		glNamedFramebufferTexture(m_FrameBuffer, GL_COLOR_ATTACHMENT1, m_Textures[COLOR_GLOSS_TEX], 0);
		glNamedFramebufferTexture(m_FrameBuffer, GL_DEPTH_STENCIL_ATTACHMENT, m_Textures[DEPTH_STENCIL_TEX], 0);

		const std::array<GLenum, std::tuple_size_v<decltype(m_Textures)>> drawBuffers{GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1};
