...
{
	FileModelData::FileModelData(std::filesystem::path path) :
		ModelData{m_MeshData}, Path{std::move(path)}
	{
	...
	std::vector<impl::MeshData> FileModelData::ProcessNodes(const aiScene* const scene) const
	{
	...
	impl::MeshData FileModelData::ProcessMesh(const aiMesh* const mesh, const aiScene* const scene, const Matrix3& trafo) const
	{
