...
#include <string>
std::string leopph::impl::Shader::s_LightPassFragmentSource{ R"#fileContents#(#version 460 core
...
vec3 CalculateLightEffect(vec3 direction, vec3 fragPos, vec3 normal, vec3 matDiff, vec3 matSpec, float matShiny, vec3 lightDiff, vec3 lightSpec)
{
	...
	{
		// Blinn-Phong
		vec3 halfway = normalize(direction + normalize(cameraPosition - fragPos));
...
vec3 CalculateDirLight(DirLight dirLight, vec3 fragPos, vec3 surfaceNormal, vec3 materialDiffuseColor, vec3 materialSpecularColor, float matShiny)
{
	...
	vec3 light = CalculateLightEffect(directionToLight, fragPos, surfaceNormal, materialDiffuseColor, materialSpecularColor, matShiny, dirLight.diffuseColor, dirLight.specularColor);
	return /*shadow */ light;
}


vec3 CalculatePointLight(PointLight pointLight, vec3 fragPos, vec3 surfaceNormal, vec3 materialDiffuseColor, vec3 materialSpecularColor, float matShiny)
{
	...
	vec3 light = CalculateLightEffect(directionToLight, fragPos, surfaceNormal, materialDiffuseColor, materialSpecularColor, matShiny, pointLight.diffuseColor, pointLight.specularColor);
	return attenuation * light;
}


vec3 CalculateSpotLight(SpotLight spotLight, vec3 fragPos, vec3 surfaceNormal, vec3 materialDiffuseColor, vec3 materialSpecularColor, float matShiny)
{
	...
	if (intensity > 0)
	{
		...
		vec3 light = CalculateLightEffect(directionToLight, fragPos, surfaceNormal, materialDiffuseColor, materialSpecularColor, matShiny, spotLight.diffuseColor, spotLight.specularColor);
...
void main()
{
    ...
    vec3 specularColor = texture(specularTexture, inTexCoords).rgb;
	float specularShine = texture(shineTexture, inTexCoords).r;

    vec3 colorSum = ambientColor;

    if (existsDirLight)
	{
		colorSum += CalculateDirLight(dirLight, position, normal, diffuseColor, specularColor, specularShine);
	}

	for (int i = 0; i < pointLightCount; i++)
	{
		colorSum += CalculatePointLight(pointLights[i], position, normal, diffuseColor, specularColor, specularShine);
	}

	for (int i = 0; i < spotLightCount; i++)
	{
		colorSum += CalculateSpotLight(spotLights[i], position, normal, diffuseColor, specularColor, specularShine);
	}
