...
Material::Material(Vector3 const& albedoVector, float const metallic, float const roughness, float const ao, ObserverPtr<Texture2D> const albedoMap, ObserverPtr<Texture2D> const metallicMap, ObserverPtr<Texture2D> const roughnessMap, ObserverPtr<Texture2D> const aoMap, ObserverPtr<Texture2D> const normalMap) :
  mShaderMtl{
    .albedo = albedoVector,
    .metallic = metallic,
    .roughness = roughness,
...
auto Material::Serialize(std::vector<std::uint8_t>& out) const noexcept -> void {
  ...
  auto const trySerializeMap{
    [&out](ObserverPtr<Texture2D const> const map) {
      if (map) {
