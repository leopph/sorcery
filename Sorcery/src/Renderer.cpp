...
auto StartUp() -> void {
  ...
  if (FAILED(gResources->dxgiDevice->GetAdapter(dxgiAdapter.GetAddressOf()))) {
...
auto GetInFlightFrameCount() noexcept -> int {
  return gInFlightFrameCount;
}


auto SetInFlightFrameCount(int const count) -> void {
  gInFlightFrameCount = std::clamp(count, MIN_IN_FLIGHT_FRAME_COUNT, MAX_IN_FLIGHT_FRAME_COUNT);

  if (FAILED(gResources->dxgiDevice->SetMaximumFrameLatency(gInFlightFrameCount))) {
