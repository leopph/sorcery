...
LEOPPHAPI [[nodiscard]] auto GetFullTime() noexcept -> float;
// The time in seconds it took to complete the previous frame
LEOPPHAPI [[nodiscard]] auto GetFrameTime() noexcept -> float;

// Defaults to -1, which means run as fast as possible
LEOPPHAPI [[nodiscard]] auto GetTargetFrameRate() noexcept -> int;
