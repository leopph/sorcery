	...
	for (auto const cam : cameras) {
		...
		auto const projMat{
			cam->GetType() == RenderCamera::Type::Perspective ?
				Matrix4::PerspectiveAsymZLH(RenderCamera::HorizontalPerspectiveFovToVertical(cam->GetHorizontalPerspectiveFov(), aspectRatio), aspectRatio, cam->GetNearClipPlane(), cam->GetFarClipPlane()) :
