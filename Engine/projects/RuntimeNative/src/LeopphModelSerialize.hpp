...
namespace leopph {
	...
	auto Serialize(MaterialData const& mat, std::vector<u8>& oBuf, std::endian endianness) -> void;
	template <class T, u64 N>
	auto Serialize(Vector<T, N> const& vec, std::vector<u8>& oBuf, std::endian endianness) -> void;
	auto Serialize(Vertex const& vert, std::vector<u8>& oBuf, std::endian endianness) -> void;
	auto Serialize(MeshData const& mesh, std::vector<u8>& oBuf, std::endian endianness) -> void;
	auto Serialize(ModelData const& model, std::vector<u8>& oBuf, std::endian endianness) -> void;
	auto Serialize(AABB const& aabb, std::vector<u8>& oBuf, std::endian endianness) -> void;
