...
{
	GlCascadedShadowMap::GlCascadedShadowMap()
	{
	...
	auto GlCascadedShadowMap::Clear() const -> void
	{
	...
	auto GlCascadedShadowMap::BindForWriting(std::size_t const cascadeIndex) const -> void
	{
	...
	auto GlCascadedShadowMap::BindForReading(ShaderProgram& shader, std::string_view const uniformName, GLuint texUnit) const -> GLuint
	{
	...
	auto GlCascadedShadowMap::CascadeMatrix(Frustum const& frustum, std::span<CascadeBounds const> cascadeBounds, Matrix4 const& worldTolightMat, Matrix4 const& camToLightMat, float const bBoxNearOffset) -> std::span<Matrix4>
	{
	...
	auto GlCascadedShadowMap::CalculateCascadeBounds(float const near, float const far) -> std::span<CascadeBounds>
	{
	...
	auto GlCascadedShadowMap::ConfigCascades(std::span<std::size_t const> const resolutions) -> void
	{
		m_Cascades.clear();
