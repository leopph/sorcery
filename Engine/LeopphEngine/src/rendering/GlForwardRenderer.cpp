...
namespace leopph::internal
{
	void GlForwardRenderer::Render()
	{
		...
		*reinterpret_cast<UboGenericData*>(lightingUbo) = genericData;
		...
		if (auto const* const shader = mForwardShaderFamily.get_current_permutation())
		{
			shader->use();
		...
		if (auto const& background = (*GetMainCamera())->Background(); std::holds_alternative<Skybox>(background))
		{
			...
			if (auto const shader = mSkyboxShaderFamily.get_current_permutation())
			{
				shader->use();
		...
		if (auto const* const shader = mTransparencyCompositeShaderFamily.get_current_permutation())
		{
			shader->use();
			DrawScreenQuad();
		}
		...
		if (auto const* const shader = mGammaCorrectShaderFamily.get_current_permutation())
		{
			shader->use();
