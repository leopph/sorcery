  ...
  // Create the vertex shader
  {
    static const char* vertexShader =
      "cbuffer vertexBuffer : register(b0) \
            ...
            \
            float GammaToLinearScalar(const float val) {\
              return val <= 0.04045 ? val / 12.92 : pow((val + 0.055) / 1.055, 2.4);\
            }\
            \
            float4 GammaToLinearFloat4(const float4 col) {\
              float4 ret = col;\
              ret.rgb *= ret.a;\
              ret.r = GammaToLinearScalar(ret.r);\
              ret.g = GammaToLinearScalar(ret.g);\
              ret.b = GammaToLinearScalar(ret.b);\
              ret.a = 1 - GammaToLinearScalar(1 - ret.a);\
              return ret;\
            }\
            \
            ...
            {\
              ...
              output.col = GammaToLinearFloat4(input.col);\
              output.uv  = input.uv;\
              return output;\
            }";
