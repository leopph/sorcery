...
namespace leopph {
auto editor::CubemapLoader::GetSupportedExtensions() const -> std::span<std::string const> {
...
auto editor::CubemapLoader::Load(std::filesystem::path const& src, [[maybe_unused]] std::filesystem::path const& cache) -> std::unique_ptr<Object> {
  int width;
  int height;
  int channelCount;
...
auto editor::CubemapLoader::GetPrecedence() const noexcept -> int {
  return 0;
}
}
